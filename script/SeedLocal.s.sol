...
contract SeedLocal is Script {
    ...
    ///
    ///      The `X_` prefix names the source rather than the metric: both values come from an X
    ///      account, leaving room for Kaito's smart-follower count to register separately later.
    ///      These names must match `SCHEMA_REACH` / `SCHEMA_FOLLOWERS` in `boneyscore.ts` — schema
    ///      ids are `keccak256(name)`, so renaming one side alone repoints every read at a schema
    ///      that does not exist, and `valueOf` returns 0 for an unknown id with no error to catch.
    ///
