...
contract BoneyScoreTest is Test {
    ...
    function test_Join_belowSeededScore_revertsInsufficientReputation() public {
        ...
        vm.prank(kol2);
        vm.expectRevert(
            abi.encodeWithSelector(Errors.InsufficientReputation.selector, 14_863, GATED_MIN_REPUTATION)
        );
        gated.join();
    }
    ...
    function test_Join_unattestedWallet_revertsWithZeroScore() public {
        ...
        vm.prank(fresh);
        vm.expectRevert(
            abi.encodeWithSelector(Errors.InsufficientReputation.selector, 0, GATED_MIN_REPUTATION)
        );
        gated.join();
    }
    ...
    function test_Join_reachAlone_revertsAtTheGate() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.InsufficientReputation.selector,
    ...
    function test_Join_followersCannotOpenTheGate() public {
        ...
        vm.prank(kol1);
        vm.expectRevert(abi.encodeWithSelector(Errors.InsufficientReputation.selector, score, score + 1));
        gated.join();
    ...
    function test_Join_expiredEthos_closesTheGateAtTheContract() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.InsufficientReputation.selector, REACH_WEIGHT * KOL1_REACH, GATED_MIN_REPUTATION
            )
        );
        afterExpiry.join();
    }
    ...
    function test_Create_gateAboveBoneyScoreCeiling_reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                Errors.UnreachableReputation.selector, MAX_BONEY_SCORE + 1, MAX_BONEY_SCORE
            )
        );
    ...
    function testFuzz_JoinGate_agreesWithTheOffChainFormula(uint256 ethos, uint256 reach) public {
        ...
        if (predicted >= GATED_MIN_REPUTATION) {
            ...
            vm.expectRevert(
                abi.encodeWithSelector(
                    Errors.InsufficientReputation.selector, predicted, GATED_MIN_REPUTATION
                )
            );
            gated.join();
        }
