...
///      to a promoter id within a campaign, and a relayer submits it.
interface IAttributionRegistry { 
    ...
    /// @notice Emitted when a user signs attribution to a promoter. 
    ...
    /// @param signedAt When the user signed.
    /// @param expiresAt When the user touch expires and stops crediting.
    ...
    /// @param promoterId id of the promoter (KOL) the user endorses.
    /// @param signedAt Timestamp the user signed at.
    /// @param expiresAt Timestamp after which this touch must be ignored and expires.
    struct Touch {
        address campaign;
        bytes32 promoterId;
        uint64 signedAt;
    ...
    /// @param expiresAt When it expires and stops crediting.
    /// @param storedAtBlock Block the touch was stored in. 
    struct TouchRecord {
        bytes32 promoterId;
        uint64 signedAt;
        uint64 expiresAt;
    ...
    /// @notice Bind a promoter id under the calling campaign.
    ...
    /// @dev       Returns `bytes32(0)` when there is no live touch.
    ...
    /// @notice The stored touch for a user in a campaign, expired or not.
    /// @param campaign The campaign to query.
    /// @param user The end user.
    ...
    /// @param campaign The campaign to query.
    /// @param user The end user.
    /// @param sinceBlock First block of the span.
