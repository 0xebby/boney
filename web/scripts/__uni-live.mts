/** Reads seeded Uniswap specs, ladders, and verifier configuration. */
...
type KpiSpec = {params: Hex; kind: number; target: bigint; aggregate: boolean; verifier: Hex};
type Tier = {threshold: bigint; reward: bigint};
type VerifierConfig = {targetContract: Hex; eventSignature: string; userParamIndex: number; aggregation: number; valueParamIndex: number; scale: bigint; windowStartBlock: bigint; windowEndBlock: bigint};
type GuardConfig = {projectVerifier: Hex; toleranceBps: number; mode: number};
for (let i = 0n; i < count; i++) {
  const spec = await read<KpiSpec>("kpi", [i]);
  const tiers = await read<Tier[]>("tiers", [i]);
  ...
  console.log(`  ladder: ${tiers.map((t) => `${t.threshold}->${t.reward / 10n ** 18n}`).join("  ")}`);
  ...
  console.log(`  guard: projectVerifier=${guard.projectVerifier}  tolerance=${guard.toleranceBps}  mode=${guard.mode}`);
}
