...
import {campaignName, hasCampaignName} from "./campaignName";
...
describe("campaignName", () => {
  it("returns the campaign's own name", () => {
    expect(campaignName(view("Aerodrome"))).toBe("Aerodrome");
    expect(hasCampaignName(view("Aerodrome"))).toBe(true);
  });
  ...
  it("keeps the capitalisation and spacing the creator chose", () => {
    ...
    expect(campaignName(view("aAvE v3"))).toBe("aAvE v3");
  });

  it("falls back to the shortened address for an unnamed campaign", () => {
    expect(campaignName(view(""))).toBe("0xba95…976A");
    expect(hasCampaignName(view(""))).toBe(false);
  });
  ...
  it("never returns an empty string", () => {
    for (const name of ["Aerodrome", "", "   ", "a"]) {
      expect(campaignName(view(name)).length).toBeGreaterThan(0);
    }
  });
  ...
  it("uses the address of the row it was handed when falling back", () => {
    ...
    expect(campaignName(view("", other))).toBe("0x9840…b2f8");
    // A named campaign resolves by name alone, so the address is irrelevant to the result.
    expect(campaignName(view("Moonwell", other))).toBe("Moonwell");
  });
  ...
  it("gives campaigns from one project their own names", () => {
    ...
    const names = ["Aerodrome", "Velodrome", "Moonwell"].map((n) => campaignName(view(n, PROJECT)));
